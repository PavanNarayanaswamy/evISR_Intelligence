...
class JmisbDecoder:
    ...
    def align_to_klv_key(self, byte_data):
        ...
        if offset == -1:
            return None
        if offset > 0:
            # print(f"Synced: Skipped {offset} bytes to find KLV header.")
    ...
    def decode_file(self, klv_path):
        ...
        if not clean_bytes:
            print("Warning: No valid KLV header found in this segment.")
        ...
        try:
            ...
            print(f"Error parsing KLV bytes: {e}")
            # Optional: Print hex to debug if it fails again
            # print("Failed Header Hex:", clean_bytes[:16].hex())
